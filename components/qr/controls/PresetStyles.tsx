    ...
    <div className="grid grid-cols-2 gap-2">
      {PRESET_STYLES.map(preset => (
        <button
          ...
          className="group flex flex-col items-start p-3 rounded-xl border-2 border-slate-200 hover:border-primary bg-slate-50 hover:bg-primary/5 transition-all text-left"
        >
          {/* Color swatches */}
          <div className="flex gap-1 mb-2">
            {preset.swatchColors.map((color, i) => (
              <div
                key={i}
                className="w-5 h-5 rounded-full border border-slate-250 shadow-sm"
                style={{ backgroundColor: color }}
              />
            ))}
          </div>
          <span className="text-xs font-bold text-[#001B50] group-hover:text-primary transition-colors">
            {preset.name}
          </span>
