...
export default function EditReviewQrPage() {
  ...
  const [shortCode, setShortCode] = useState('preview');

  useEffect(() => {
    const fetchCampaignDataAndGeneral = async () => {
      try {
        ...
        // 2. Fetch specific Review QR details
        const qrRes = await fetch(`/api/qrs/${id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!qrRes.ok) throw new Error('Failed to load campaign settings.');
        const qrData = await qrRes.json();
        
        const qr = qrData.qrCode;
        if (qr.type !== 'REVIEW') {
          throw new Error('This is not a review campaign QR.');
        }

        setName(qr.name);
        setCampaignId(qr.campaignId?.toString() || '');
        ...
        const dest = qr.destination as any;
        setBusinessName(dest.businessName || '');
        setLogoUrl(dest.logoUrl || '');
        setPublicReviewUrl(dest.publicReviewUrl || '');
        setPositiveThreshold((dest.positiveThreshold || 4).toString());
        setWelcomeMessage(dest.welcomeMessage || '');
        setPrivateFeedbackMessage(dest.privateFeedbackMessage || '');
        setThankYouMessage(dest.thankYouMessage || '');

      } catch (error: any) {
        console.error(error);
        toast.error(error.message || 'Could not load campaign data.');
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Review campaign updated successfully!');
      router.push('/dashboard/reviews');
    } catch (error: any) {
      console.error(error);
      toast.error(error.message || 'Failed to save campaign settings.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-slate-400 text-sm">Loading Review settings...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-[#001B50]">Edit Review QR Code</h2>
          <p className="text-slate-500 text-xs mt-0.5">Modify smart redirect parameters and business details</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 focus:border-primary focus:ring-1 focus:ring-primary rounded-xl py-3 px-4 text-sm text-slate-800 outline-none transition-all"
                required
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 focus:border-primary rounded-xl py-3 px-4 text-sm text-slate-705 outline-none cursor-pointer transition-all"
              >
